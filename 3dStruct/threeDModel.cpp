...
float ThreeDModel::lengthV(glm::vec3 point, Vector3d vertex) {
	return sqrt(((point.x - vertex.x) * (point.x - vertex.x)) + ((point.z - vertex.z) * (point.z - vertex.z)));
}

std::vector<float> ThreeDModel::checkPointOctree(glm::vec3 point) {

	std::vector<int> result = octree->checkPoint(point);
	...
	if (result.size() >= 3) {
		...
		for (int i = 0; i < result.size(); i++) {
			...
			pair p = { result[i] ,lengthV(point, theVerts[result[i]]) };
			dist.push_back(p);
		}

		std::sort(dist.begin(), dist.end(), [](auto const& a, auto const& b) { return a.dist < b.dist; });

		Vector3d face[3] = { theVerts[dist[0].vert],theVerts[dist[1].vert] ,theVerts[dist[2].vert] };

		Vector3d edge1 = face[0] - face[1];
		Vector3d edge2 = face[0] - face[2];
		...
		float distance = abs(glm::dot(glm::vec3(point.x-face[0].x, point.y - face[0].y, point.z - face[0].z), glm::vec3(normal.x, normal.y, normal.z)));
		cout << distance << endl;
		if (distance < 0.001) {
			ans.push_back(normal.x);
			ans.push_back(normal.y);
			ans.push_back(normal.z);
...
bool ThreeDModel::checkPointOctree(glm::vec3 point,int a) {
	return octree->checkPoint(point,a);
}