...
std::vector<int> Octree::checkPoint(glm::vec3 point)
{
	std::vector<int> test;
	if (Level >= MAX_DEPTH) //leaf
	{
		if (PrimitiveListSize > 0) //only check boxes which contain vertices
		{
			//return true if in leaf min max box thing
			if ((point.x<maxX && point.x > minX && point.y<maxY && point.y > minY && point.z<maxZ && point.z > minZ)) {
	
				//cout << "i am here" << endl;
				int temp = PrimitiveListSize;
				for (int i = 0; i < temp; i++) {
					int hhhhhhhhhhhhh = PrimitiveList[i];
					
				}
				std::vector<int> list(VertexList, VertexList + VertexListSize);
				test = list;
				return test;
			}
		}
	...
	else
	{
		if ((point.x<maxX && point.x > minX && point.y<maxY && point.y > minY && point.z<maxZ && point.z > minZ)) {
			...
			for (int i = 0; i < 8; i++)
			{
				if (children[i] != NULL)
				{
					//is the point in child i
					test = children[i]->checkPoint(point);
					if (test.size()>0)
						return test;
				}
			}
...
bool Octree::checkPoint(glm::vec3 point,int a)
{
	if (Level >= MAX_DEPTH) //leaf
	{
		if (PrimitiveListSize > 0) //only draw boxes which contain vertices
		...
		if ((point.x<maxX && point.x > minX && point.y<maxY && point.y > minY && point.z<maxZ && point.z > minZ)) {
			for (int i = 0; i < 8; i++)
			{
				if (children[i] != NULL)
				{
					//is the point in child i
					int a = 0;
					if (children[i]->checkPoint(point, a))
						return true;
